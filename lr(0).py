...
while countI<len(omegaList):
    ...
    for i in vars_in_I:
        ...
        if (In[0][1][-1]!="."):
            ...
            print(f'Action(I{countI-1},{i}):{temp} This is basically I{omegaList.index(temp)}')
        else:
            temp = set(In)
            if temp not in omegaList:
                omegaList.append(temp)
            if i in non_terms:
                ...
                newrow[i] = "s"+str(omegaList.index(temp))
            print(f'Action(I{countI-1},{i}):{temp} That is I{omegaList.index(temp)}')

    stateTable.append(newrow)
print("\n\nCanonical Items and Augmented grammar are as follows: \n")
for i in omegaList:
